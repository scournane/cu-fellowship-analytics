...
oauth_config:
  scopes:
    bot:
      ...
      - chat:write            # Q&A pointers and summaries; reminders, badge DMs, staff posts
      - app_mentions:read     # Q&A: "@bot summary" in a channel
      - im:write              # open a DM to send a reminder or a welcome
      - commands              # the slash commands (second half of this document)
  slash_commands:             # one entry per command; the URL is ignored in Socket Mode
    - command: /reminders
      description: See or change when you get session and assignment reminders
    - command: /badges
...
settings:
  event_subscriptions:
    bot_events:
      ...
      - team_join             # roster alert when someone new joins the workspace
  interactivity:
    is_enabled: true          # the "check in with me" button
...
session summaries → the Monday digest.
