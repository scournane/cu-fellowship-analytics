    ...
    <Stack gap={4}>
      ...
      <Card padding={5}>
        <Stack as="form" method="post" action={action} gap={4}>
          ...
          <Stack gap={1}>
            <Heading level={2}>Exit ticket (Part A)</Heading>
            <Text type="supporting">
              {session_id
                ? 'This session asks its cohort’s default exit ticket questions unless it has its own. Change them from the session page, any time before its Part A form is published.'
                : 'A new session asks its cohort’s default exit ticket questions. Once it is saved you can give it its own, from the session page, any time before its Part A form is published.'}
            </Text>
            {session_id ? (
              <Stack direction="horizontal">
                <Link href={`/sessions/${session_id}/questions`}>{'See this session’s questions'}</Link>
              </Stack>
            ) : null}
          </Stack>
