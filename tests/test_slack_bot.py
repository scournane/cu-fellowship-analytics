...
from cufa.config import load_settings, reset_settings_cache
...
# The quiet window these tests were written against. The deployment's is
# configurable (CUFA_SLACK_QUIET_START / END; 21:00-08:00 out of the box) and
# each fellow can set their own with /cufa-reminders.
QUIET = {"CUFA_SLACK_QUIET_START": "22:00", "CUFA_SLACK_QUIET_END": "07:00"}
...
def test_assignment_reminders_carry_the_link_and_a_failed_dm_is_retried(db, workspace):
    ...
    # A failed delivery is retried after five minutes (bot_delivery keeps the attempt).
    assert run_reminders(db, workspace, cohort_id=TEST_COHORT, now=at + timedelta(minutes=6)).sent == 1, "the failed one is not marked sent"
...
def test_zoom_link_and_assignment_flow_through_commands(db, workspace, settings):
    ...
    assert fetch_one(db, 'select zoom_url from "session"')["zoom_url"] == "https://zoom.us/j/9"
