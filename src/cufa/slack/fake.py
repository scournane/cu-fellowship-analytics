...
class FakeSlackWebClient:
    ...
    def chat_postMessage(self, *, channel: str, text: str = "", **kwargs: Any) -> dict[str, Any]:
        ...
        if channel in self.ws.users:
            ...
            # Kept in the fake so tests prove DMs are not channel posts without
            # needing im:history or any inbound DM scope. The channel becomes
            # the same D-id conversations.open would have handed out.
            recipient = channel
            channel = self.ws.dm_channel(channel)
        if channel not in self.ws.channels and recipient is None:
            raise _err("chat.postMessage", "channel_not_found")
        ts = self.ws.next_ts()
        # `to_user` is always present (None for a channel post); `to` only on a
        # DM, so `if p.get("to")` picks the DMs out of the outbox.
        entry: dict[str, Any] = {"channel": channel, "to_user": recipient, "text": text, "ts": ts}
