...
def weekly_digest_text(conn: psycopg.Connection, cohort_id: str, *, now: datetime | None = None) -> str:
    ...
    sessions = fetch_all(
        conn,
        'select title, scheduled_at_utc, timezone, zoom_url from "session" where cohort_id = %s and scheduled_at_utc between %s and %s order by scheduled_at_utc',
...
def tick(
    ...
    *,
    ...
    ``reminders=False`` is for the bot process, where the automation loop
    (``cufa.slack.reminders.AutomationLoop``) already sends them each minute;
    from cron, leave it on and this tick sends them through the same engine.
    """
    ...
    if reminders:
        run = step("reminders", lambda: run_reminders(conn, client, cohort_id=cohort_id, now=now, settings=settings))
