...
def cmd_assignment(args: argparse.Namespace) -> int:
    ...
    with connection() as conn:
        if args.assignment_action == "link":
...
def add_parsers(
    ...
    *,
    ...
    is cli.py's: it runs one automation tick, which sends reminders.
    """
    sp = slack_sub
    ...
    # --- assignment ---
    # `cufa assignment create|edit|list` are cli.py's; these are the
    # submission and score actions the bot's `/assignment` and `/score` use.
    sp = assignment_sub
    q = sp.add_parser("link", help="put the link on an assignment")
    q.add_argument("--cohort")
    q.add_argument("which", help="id, kind or title fragment")
    q.add_argument("url")
    q.set_defaults(func=cmd_assignment)
    q = sp.add_parser("submitted", help="mark that a fellow submitted")
    q.add_argument("--cohort")
    q.add_argument("which")
    q.add_argument("--fellow", required=True)
    q.set_defaults(func=cmd_assignment)
    q = sp.add_parser("score", help="record a hand-entered score")
    q.add_argument("--cohort")
    q.add_argument("which")
    q.add_argument("--fellow", required=True)
    ...
    q.add_argument("--note")
    q.set_defaults(func=cmd_assignment)
    q = sp.add_parser("show", help="one assignment with every fellow's submission and score")
    q.add_argument("--cohort")
    q.add_argument("which")
    q.set_defaults(func=cmd_assignment)
